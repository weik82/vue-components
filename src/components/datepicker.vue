    ...
    <div class="vue-datepicker">
        ...
        <div v-if="show" class="vue-datepicker-wrap">
            <div class="vue-datepicker-header" @click.stop="">
                ...
                <span @click.stop="switchMonth(1)"
                      class="vue-datepicker-header-btn vue-datepicker-header-btn-next">&gt;</span>
            </div>
            <div class="vue-datepicker-content">
                ...
                <!--<div v-if="selectYear" class="vue-date-picker-year-panel">-->
                    <!--<ul ref="year">-->
                        <!--<li v-for="y of years" @click.stop="pickYear(y)" :class="{'active': y == select.year}">{{y}}-->
                        <!--</li>-->
                    <!--</ul>-->
                    <!--<ul ref="month">-->
                    ...
                    <!--</ul>-->
                <!--</div>-->
                <div v-if="selectYear" style="position: absolute;background-color:#fff;left: 0;top: 0;width: 100%;height: 100%;display: flex;flex-flow: row wrap;">
                    <div style="cursor:pointer;flex: 0 0 25%;height: 33%;display: flex;align-items: center;justify-content: center" v-for="(m, $index) of months"
                        @click.stop="pickMonth($index + 1)"
                        :class="{'month-active': $index + 1 == select.month}">
                        {{m}}
                    </div>
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            transform(time) {
                ...
                this.currentMonthFirstDay = new Date(this.select.year, this.select.month - 1, 1).getDay();// 当前月的1号属于星期几
                this.currentMonthEndDate = new Date(this.select.year, this.select.month, 0).getDate(); // 当前月的最后一天是几号
                this.currentMonthEndDay = new Date(this.select.year, this.select.month, 0).getDay();// 当前月的最后一天属于星期几
            },
            /*
             * 计算出日历列表，二维数组
             * 第一层为星期，第二层为每星期的第几天
            ...
            getDateList() {
                ...
                this.list.push(list);
                temp = (7 - this.currentMonthFirstDay) + 1;//第二行第一天是几号

                /*
                 * 剩下的行数
                 */
                // 计算除了第一行剩下的天数
                const leftDays = this.currentMonthEndDate - temp + 1;
                // 剩下的星期数
                const lineNumber = Math.ceil(leftDays / 7);
                // 包含下个月日历的天数
                const nextMonthDays = 7 - this.currentMonthEndDay - 1;
                const nextDays = 7 - (leftDays % 7);
                for (let i = 0; i < lineNumber; i++) {
                    if (i === lineNumber - 1) {
                        this.list.push(this.numberList(temp, this.currentMonthEndDate).concat(this.numberList(1, nextMonthDays, true)))
                    } else {
            ...
            switchMonth(n) {
                ...
                this.transform(new Date(year, month + n - 1));
                /* if (n === -1) {
                 this.transform(new Date(year,this.select.month+n-1))
                 const pre = this.select.month === 1 ? 12 : this.select.month - 1;
                 if (pre === 12) {
                 this.transform(new Date(year - 1, pre - 1));
                 } else {
                 this.transform(new Date(year, pre - 1))
                 }
                 } else {
                 const next = this.select.month === 12 ? 1 : this.select.month + 1;
                 if (next === 1) {
                 this.transform(new Date(year + 1, next - 1))
                 } else {
                 this.transform(new Date(year, next - 1))
                 }
                 }*/
            },
            pick(day) {
            ...
            // 选取年
            pickYear(n) {
                this.transform(new Date(n, this.select.month - 1));
//                this.complete()
            },
            // 选取月
            pickMonth(n) {
                this.transform(new Date(this.select.year, n - 1));
//                this.complete()
